...
use rsync_core::pg::{change_revision_status, create_revision, ensure_repository, RevisionStatus};
...
fn mock_s3_presign_map(namespace: &str, entries: &[(Vec<u8>, Metadata)]) -> BTreeMap<String, Uri> {
...
async fn integration_test(pool: PgPool) {
    ...
    insert_to_revision(rev, filelist, &pool).await;
